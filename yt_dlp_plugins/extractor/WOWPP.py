...
class WOWPPIE(InfoExtractor):
    _LOGIN_URL = 'https://www.wowpresentsplus.com/login'
    _NETRC_MACHINE = 'wowpresentsplus'

    _VALID_URL = r'https?://(?:www\.)?wowpresentsplus\.com/(?:[^/]+/)*videos/(?P<id>[^/]+)/?$'
    ...
    def _real_extract(self, url):
        ...
        webpage = None
        if self._get_cookies('https://www.wowpresentsplus.com').get('_session'):
        ...
        return {
            ...
            'url': VHXEmbedIE._smuggle_referrer(embed_url, 'https://www.wowpresentsplus.com'),
...
class WOWPPSeasonIE(InfoExtractor):
    _PAGE_SIZE = 24
    _VALID_URL = r'https?://(?:www\.)?wowpresentsplus\.com/(?P<id>[^\/$&?#]+)(?:/?$|/season:(?P<season>[0-9]+)/?$)'

    def _fetch_page(self, url, season_id, page):
        page += 1
        webpage = self._download_webpage(
            f'{url}?page={page}', season_id, note=f'Downloading page {page}', expected_status={400})
        yield from [self.url_result(item_url, WOWPPIE) for item_url in traverse_obj(
