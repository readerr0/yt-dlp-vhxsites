...
class iNDIE(InfoExtractor):
    _LOGIN_URL = 'https://watch.indieflix.com/login'
    _NETRC_MACHINE = 'indieflix'

    _VALID_URL = r'https?://(?:watch\.)?indieflix\.com/(?:[^/]+/)*videos/(?P<id>[^/]+)/?$'
    ...
    def _real_extract(self, url):
        ...
        webpage = None
        if self._get_cookies('https://watch.indieflix.com').get('_session'):
        ...
        return {
            ...
            'url': VHXEmbedIE._smuggle_referrer(embed_url, 'https://watch.indieflix.com'),
...
class iNDSeasonIE(InfoExtractor):
    _PAGE_SIZE = 24
    _VALID_URL = r'https?://(?:watch\.)?indieflix\.com/(?P<id>[^\/$&?#]+)(?:/?$|/season:(?P<season>[0-9]+)/?$)'

    def _fetch_page(self, url, season_id, page):
        page += 1
        webpage = self._download_webpage(
            f'{url}?page={page}', season_id, note=f'Downloading page {page}', expected_status={400})
        yield from [self.url_result(item_url, iNDIE) for item_url in traverse_obj(
